...
class Monitor extends Component {
    ...
    confirmOrder() {
        ...
        if(orders && orders.length){
            axios.post("http://localhost:3001/orders", {orderedData : new Date(), totalPrice, orders})
            .then(res => {
                this.setState({totalPrice: 0, orders : [], confirm: true, msg: 'บันทึกรายการสังซื้อเรียบร้อยแล้วค่ะ' });    
            })
        } else {
            this.setState({totalPrice: 0, orders : [], confirm: true, msg: 'เลือกอาหารก่อนค่ะ' });                
        }
